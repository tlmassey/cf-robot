function [ ] = find_holes_using_encoder_function( num_of_holes, vid )
% copied code from find_holes_step_by_step_function.m and added encoder
...
for hole = 1:TOTAL_NUM_OF_HOLES
    ...
    [xpos, ypos] = enc_position(sixk);
    %fringe = [fringe, [xpos, ypos]];
    fringe = [fringe, xpos];
    fringe = [fringe, ypos];
...
for hole = 1:TOTAL_NUM_OF_HOLES
    move_to_enc_position_function(fringe(2 * hole - 1), fringe(2 * hole));
    prompt = 'F/C/E Feeder/Continue/Exit';
    x = input(prompt);
    if x == 'F'
        feed = serial('COM12');fopen(feed);pause(3);
